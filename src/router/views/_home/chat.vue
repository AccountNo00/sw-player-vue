		...
		leftClick(data){
			this.$el.addEventListener('click', ()=>{
				if(this.checkSelected() != 0){
					...
					this.selected_messages.push(data.id)
				}
			});
		},
		closeSelected(){
			return this.selected_messages.length = 0;
		},
		checkSelected(){
			return this.selected_messages.length
		},
		getReply(data){
			this.reply.username = data.sender_username
			this.reply.message = data.message
			this.reply.toggle = 1;
			this.replyModal = false;
		},
		pinMessage(data){
			this.pin.pin_username = data.sender_username
			this.pin.pin_message = data.message
			this.replyModal = false;
		},
		copyText(data){
			...
			<div class="right-container">
				<div class="chat-box">
					<div class="title">
						...
						<div v-else-if="this.checkSelected()" class="m-3 d-flex" >
							<div class="text-white bg-info p-1" style="border-radius: 3px;width: 100px;">FORWARD: {{this.checkSelected()}}</div>
							<button class="check-close-btn text-white bg-info p-1 mx-4" style="border-radius: 3px;width: 30px;" @click="closeSelected()">X</button>
						</div>
					</div>
					<!-- <button
					...
					>
						<div class="pin-col">
							...
							<span class="text-dark mx-2">
								{{this.pin.pin_username + ':' + this.pin.pin_message}}
							</span>	
						</div>
					</div>
					<div class="chat-container col-10">
						<div
							...
							:key="index"
						>
							<span @contextmenu.prevent="replyClick(data,$event),leftClick(data)" class="message" id="reply" :class=" data.sender_id == this.profile.id ? 'bg-info text-white' : 'bg-light' ">
								{{ data.sender_username +':' +data.message }}
							</span>
							<div class="reply-modal" v-if="this.replyModal == true && data.id == this.selected_item && data.sender_id != this.profile.id" :x="xCoordinate" :y="yCoordinate"> 
								<button class="reply-buttons text-dark" @click="getReply(data)">Reply</button>
								<button class="reply-buttons text-dark" @click="copyText(data)">Copy</button>
								<button class="reply-buttons text-dark" @click="pinMessage(data)">Pin</button>
								<button class="reply-buttons text-dark">Forward</button>
								<button class="reply-buttons text-dark" @click="selectMessages(data)">Select</button>
								<button class="reply-buttons text-white bg-danger">Delete</button>
							</div>
						</div>
						<div id="scroll-to"></div>
					</div>

					<span class="reply-bg" v-if="this.reply.toggle == 1">{{this.reply.username + ':' + this.reply.message}}</span>
